...
def make_fooling_image(X, target_y, model):
  ...
  - X_fooling: An image that is close to X, but that is classifed as target_y
    by the model.
  """
  X_fooling = X.copy() # not change the input X
  ##############################################################################
  ...
  ##############################################################################
  N,_,_,_ = X_fooling.shape # N = 1
  lr = 100
  y_pred = -1
  i = 0
  while(i<200)&(y_pred != target_y):
    scores, cache = model.forward(X_fooling,mode='test') #(n,c)

    #set the dscores to be [0,0,....1,....,0], then backprop to image ppt32 lec9
    dscores = np.zeros_like(scores)
    dscores[np.range(N),target_y] = 1.0
    dX, grads = model.backward(dscores, cache)
    X_fooling += lr*dX
    y_pred = model.loss(X_fooling).argmax(axis=1) #(n,c)->(n,)
    i += 1
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
