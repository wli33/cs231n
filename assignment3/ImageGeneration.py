...
def create_class_visualization(target_y, model, **kwargs):
    ...
    for t in xrange(num_iterations):
        ...
        scores, cache = model.forward(X, mode='test') #(1,c)
        loss = scores[0,target_y] - l2_reg*np.sum(X**2)
        
        dscores = np.zeros_like(scores)
        dscores[0, target_y] = 1.0
        dX, grads = model.backward(dscores, cache)
        dX -= 2*l2_reg*X # add the regularization -R(I)

        X += learning_rate*dX #gradient ascent 
...
def invert_features(target_feats, layer, model, **kwargs):
  ...
  for t in xrange(num_iterations):
    ...
    dfeats = 2* (feats-target_feats) #backprop to dfeat 
...
def deepdream(X, layer, model, **kwargs):
  ...
  for t in xrange(num_iterations):
    ...
    ############################################################################
    feats,cache = model.forward(X,end = layer,mode = 'test')
    #set the gradient equal to the activations themselves
    dfeats = feats
    dX,_ = model.backward(dfeats, cache)
...
img = deprocess_image(out, data['mean_image'], mean='pixel')
plt.imshow(img)
plt.gcf().set_size_inches(8, 8)
plt.axis('off')
plt.imsave('medeep.jpg',img)
