...
class TwoLayerNet(object):
  ...
  def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
    ...
    # W1: D*Hidden W2: Hidden*C b1: hidden b2:num_classes
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    if self.use_batchnorm:
      for i in range(self.num_layers-1): #norm only on hidden layers after fwd
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1,self.num_layers):
      ...
      if self.use_batchnorm:
        ...
        layer[i],cache_layer[i] = affine_norm_relu_forward(layer[i-1], w, b,
                                                           gamma,beta,bn_param)
      else:
        layer[i],cache_layer[i] = affine_relu_forward(layer[i-1],w,b)

      if self.use_dropout:
         layer[i], cache_drop[i] = dropout_forward(layer[i], self.dropout_param)
    ...
    grads['W%d'%(self.num_layers)] += self.reg * W_last
    ...
    for i in reversed(range(1,self.num_layers)):
      ...
      if self.use_batchnorm:
        dx[i], dw, db, dgamma, dbeta = affine_norm_relu_backward(dx[i+1], cache_layer[i])
